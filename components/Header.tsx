    ...
    <header className="p-4 bg-black text-white shadow-md relative">
      <div className="flex justify-between items-center w-full">
        ...
        <div className="relative w-1/2">
          ...
          />
          {results.length > 0 && (
            <div className="absolute top-full left-0 w-full mt-1 bg-white text-black rounded shadow-md max-h-60 overflow-y-auto z-50">
              <ul>
                {results.map((product) => (
                  <li key={product.id} className="border-b px-4 py-2 hover:bg-gray-200">
                    <Link href={`/products/${product.id}`}>
        ...
        <Link href="/cart" className="relative">
          <FaCartArrowDown size={24} className="hover:text-gray-400" />
          <span className="absolute -top-2 -right-3 bg-red-500 text-white text-xs rounded-full px-1.5"></span>
        </Link>
      </div>
      {loading && <p className="text-gray-400 mt-2 text-center">Loading...</p>}
    </header>
